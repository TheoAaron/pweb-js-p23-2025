...
// Recipes page script
(function(){
  const API = 'https://dummyjson.com/recipes?limit=1000'; // ambil semua data
  ...
  function hideError(){ if (!errorEl) return; errorEl.style.display = 'none'; errorEl.textContent = ''; }
  ...
  async function fetchRecipes(){
    try{
      ...
      hideError(); // pastikan error lama disembunyikan
  ...
  function populateCuisineOptions(list){
    // reset agar tidak double append jika dipanggil ulang
  ...
  function renderCard(r){
    ...
    const prepMins = r.prepTimeMinutes ?? undefined; // dipakai di modal
    ...
    el.innerHTML = `
      ...
      <div class="card-body">
        ...
        <div class="card-footer">
          <div class="rating">${renderStars(r.rating || 0)}</div>
          <div>
  ...
  function showModal(r){
    ...
    const steps = Array.isArray(r.instructions)
      ? r.instructions
      : (typeof r.instructions === 'string' ? r.instructions.split('\n').filter(Boolean) : []);
  ...
  function updateResultsInfo(){
    ...
    if (!filtered.length) { resultsInfo.textContent = 'No results.'; return; }
    resultsInfo.textContent = `Showing ${Math.min(offset, filtered.length)} of ${filtered.length} recipes`;
  }
